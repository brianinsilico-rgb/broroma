...
const controlValveImages: Record<string, string> = {
...
export default function ControlValvesCategoryPage() {
  const { t } = useLanguage();
  const { hero, cta, products } = t.products.controlValves;
            ...
            {products.map((product: { slug: string; name: string; description: string }) => (
              <Link key={product.slug} href={`/products/control-valves/${product.slug}`} className="group">
                <div className="relative aspect-[4/3] rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-shadow duration-300">
                  <Image src={controlValveImages[product.slug] ?? ""} alt={product.name} fill className="object-cover group-hover:scale-105 transition-transform duration-300" />
