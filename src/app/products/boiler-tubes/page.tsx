...
const materialSpecs = [
  ...
  { material: "Stainless Steel", grades: "TP304, TP304H, TP316, TP321", sizes: "1/4\" - 3\"", standard: "ASTM A213" },
...
export default function BoilerTubesPage() {
  ...
  const applications = (boiler?.applications as string[] | undefined) ?? APPLICATIONS;
        ...
        <div className="container-custom">
          <div className="mb-6">
            <h2 className="text-xl md:text-2xl font-semibold text-navy-900 mb-2">
              {boiler?.materialsTitle ?? "Boiler Tube Materials"}
            </h2>
            <p className="text-gray-600 text-sm md:text-base max-w-3xl">
              {boiler?.materialsDescription ?? "Carbon, stainless, alloy, and duplex steel tubes for boiler and steam applications."}
            </p>
          </div>
          <SpecsTable rows={materialSpecs} />
          <div className="mt-5">
            <PdfDownloadLink href={boiler?.downloads?.catalogPdf ?? "#"} label={boiler?.downloads?.catalogLabel ?? "Download Boiler Tubes Catalog (PDF)"} />
          </div>
        </div>
      </section>
      ...
      <section className="py-10 md:py-16 bg-white">
        <div className="container-custom">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <DownloadCard href={boiler?.downloads?.catalogPdf ?? "#"} title={boiler?.downloads?.catalogName ?? "Boiler Tubes Catalog"} format="PDF" />
            <DownloadCard href={boiler?.downloads?.fullPdf ?? "#"} title={boiler?.downloads?.fullCatalogName ?? "Full Tubes Catalog"} format="PDF" />
          </div>
        </div>
      </section>
        ...
        <div className="container-custom">
          <div className="bg-gradient-to-br from-navy-900 to-navy-950 rounded-2xl p-6 md:p-10 text-center">
            <h2 className="text-lg font-semibold text-white mb-2 md:text-3xl lg:text-4xl md:mb-4">
              {boiler?.cta?.title ?? "Need a Quote for Boiler Tubes?"}
            </h2>
            <p className="text-navy-200 text-sm mb-5 max-w-xl mx-auto md:text-lg md:mb-8 md:max-w-2xl">
              {boiler?.cta?.description ?? "Contact our team for pricing, availability, and custom specifications."}
            </p>
